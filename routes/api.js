...
module.exports = function (app) {
    ...
    .get((req, res) => {
      Message.find({board: req.params.board})
        ...
        .exec((err, messages) => {
          ...
          let messagesConsolidate = messages.map(message => {
            let container = {};
  ...
  app.route('/api/replies/:board')
    ...
    .put((req, res) => {
      Message.findOne({board: req.body.board, _id: mongoose.Types.ObjectId(req.body.thread_id)}, async (err, doc) => {
        if (err) throw err;
        if (!doc) res.send("invalid thread ID");
        else {
          const result = doc.replies.findIndex(({_id}) => {
            return _id == req.body.reply_id;
          })
          if (isNaN(result)) res.send("invalid reply ID");
          else {
